    ...
    if (direction == LEFT && x == 1) {
        ...
        for (int itr = 0; itr < 4; itr++) {
            if (itr == direction - 2 || itr == direction + 2)
                continue;
            if (map[y + g_y_inc[direction] + g_y_inc[itr]]
                   [x + g_x_inc[direction] + g_x_inc[itr]] != wall_char) {
                if (g_y_inc[itr] == 1)
...
void Mazegen::extend_left() { new_yline(0); }

void Mazegen::extend_right() { new_yline(mapsize_x - 2); }

void Mazegen::extend_top() { new_xline(0); }

void Mazegen::extend_bottom() { new_xline(mapsize_y - 2); }
    ...
    for (size_t itr_y = 1; itr_y < mapsize_y - 1; itr_y++) {
        ...
        for (size_t itr_x = 1; itr_x < mapsize_x - 1; itr_x++) {
            if (itr_y % 2 == 1) {
                if (itr_x % 2 == 1)
                    newmap[itr_y][itr_x] =
                        map[(itr_y - 1) / 2 + 1][(itr_x - 1) / 2 + 1];
                else {
                    if (map[(itr_y - 1) / 2 + 1][(itr_x - 1) / 2 + 1] ==
                            blank_char &&
